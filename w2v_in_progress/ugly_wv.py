##word2vec implementation up to Jan 12
...
def generate_batch(num_skips, batch_size, data, start_index):
    ...
    if (start_index + num_center_words)<=len(ex_indices):
        #All required data in bounds of ex_indices
        center_word_indices = ex_indices[start_index:start_index+num_center_words]
    else:
        #loop around to beginning of ex_indices
        extra_words = (start_index + num_center_words) - len(ex_indices)
        center_word_indices = ex_indices[start_index:]
        center_word_indices.extend(ex_indices[:extra_words])
